...
TEST(PushRole, First)
{
    EXPECT_NO_THROW(registerService("private.A", false));
    EXPECT_NO_THROW(registerService("private.A", true));
    ...
    EXPECT_NO_THROW(registerService("private.X", true));
    EXPECT_NO_THROW(registerService("public.Y", true));
    EXPECT_NO_THROW(registerService("public.Y", false));
}
...
TEST(PushRole, PushRole)
{
...

    {
        ...
        EXPECT_NO_THROW(prv.pushRole((conf_root + "/private.A.json").c_str()));
        EXPECT_NO_THROW(pub.pushRole((conf_root + "/public.B.json").c_str()));
